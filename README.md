...
批量选择会话进行归档或删除的管理面板（DSH Web GUI 设置页新区块）。

## 功能

设置页新增「会话管理」区块（`settings.section` slot，id `session-batch-manager`）：

- **会话列表**：列出本机全部会话（官方 `POST /api/session.list` + `POST /api/workspace.list` 合并），
  每行显示 标题（projection `title`）/ 状态（运行中 · 归档 · 空闲）/ cwd，subagent 会话额外标注；
  含已归档会话与冷会话。
- **多选**：checkbox 逐行勾选 + 全选。
...
- **client**（`src/client/index.ts`）：`settings.section` 注册 + 原生 DOM 面板（无 UI 框架）；
...
# typecheck：DSH_CHECKOUT=... bash scripts/build.sh（tsc --noEmit 等价检查在构建内执行）
...
- **运行中 / subagent 会话拒删**：批量删除自动跳过，UI 汇总展示跳过原因。
